...
def test(file):
    ...
    y, x = svm_read_problem(file)
    total_len = len(y)
    sp1 = int(0.3 * total_len)
    sp2 = int(0.4 * total_len)
    m = svm_train(y[:sp1] + y[sp2:], x[:sp1] + x[sp2:], param)
    p_label, p_acc, p_val = svm_predict(y[sp1:sp2], x[sp1:sp2], m)
    fs = fscore(p_label, y[sp1:sp2])
    return fs
...
if __name__ == '__main__':
    # score = cross_validation(r'..\data\data_balanced\acous_lex_bigram.txt')
    score = test(r'..\data\data_balanced\acous_lex_bigram.txt')
    print(score)